    ...
    def auth_required_response(self) -> http.HTTPResponse:
        if self.is_proxy_auth():
            return http.make_error_response(
                ...
                headers=seleniumwire.thirdparty.mitmproxy.net.http.Headers(Proxy_Authenticate=f'Basic realm="{REALM}"'),
            )
        else:
            return http.make_error_response(
                status_codes.UNAUTHORIZED,
                headers=seleniumwire.thirdparty.mitmproxy.net.http.Headers(WWW_Authenticate=f'Basic realm="{REALM}"'),
            )
